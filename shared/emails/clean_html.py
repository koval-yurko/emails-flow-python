...
def clean_html(html_string: str, options: Optional[Dict[str, Any]] = None) -> str:
    ...
    return clean_html_content(html_string, {
...
def extract_text_from_element(soup: BeautifulSoup) -> str:
    ...
    text_content = re.sub(r'^\s+|\s+$', '', text_content, flags=re.MULTILINE)  # Remove leading/trailing spaces from lines
...
def clean_html_content(html: str, options: Dict[str, Any]) -> str:
    ...
    cleaned = re.sub(r'\s*class\s*=\s*["\'][^"\']*["\']', '', cleaned, flags=re.IGNORECASE)
